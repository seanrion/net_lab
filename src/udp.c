...
{
    udp_hdr_t* udphead = (udp_hdr_t*)buf->data;
    buf_add_header(buf,sizeof(udp_peso_hdr_t));
    udp_peso_hdr_t* head = (udp_peso_hdr_t*)buf->data;
    udp_peso_hdr_t temp = *head;
    memcpy(head->src_ip,src_ip,NET_IP_LEN);
    memcpy(head->dest_ip,dest_ip,NET_IP_LEN);
    head->placeholder = 0;
    head->protocol = NET_PROTOCOL_UDP;
    head->total_len = udphead->total_len;
    uint16_t checksum = checksum16((uint16_t*)buf->data,swap16(udphead->total_len)+sizeof(udp_peso_hdr_t));
    *head = temp;
...
void udp_in(buf_t *buf, uint8_t *src_ip)
{
    ...
    if(swap16(head->total_len) > 1480){
        return;
    }
    uint16_t checksum = head->checksum;
    head->checksum = 0;
    head->checksum = udp_checksum(buf,src_ip,net_if_ip);
    if(head->checksum != checksum){
        return;
    }
    for (int i = 0; i < UDP_MAX_HANDLER; i++) {
        if (udp_table[i].port == swap16(head->dest_port) && udp_table[i].valid == 1){
...
void udp_out(buf_t *buf, uint16_t src_port, uint8_t *dest_ip, uint16_t dest_port)
{
    ...
    head->checksum = 0;
    udp_checksum(buf, net_if_ip, dest_ip);
